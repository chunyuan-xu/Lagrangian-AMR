...
// corner k, the 2x2 block is stored in master.M[k][0..3] and RHS in
// master.b[2k..2k+1].
...
inline void write_cell_local_master(quad_data_t &data)
{
	for (int c = 0; c < kQuadCornerCount; ++c) {
		...
		data.nodal.master.M[c][0] = out.M.m[0][0];
		data.nodal.master.M[c][1] = out.M.m[0][1];
		data.nodal.master.M[c][2] = out.M.m[1][0];
		data.nodal.master.M[c][3] = out.M.m[1][1];
		data.nodal.master.b[2 * c] = out.b[0];
		data.nodal.master.b[2 * c + 1] = out.b[1];
	}
}

inline bool verify_local_master_against_legacy(const quad_data_t &data,
	double tol = 1e-9)
{
	for (int c = 0; c < kQuadCornerCount; ++c) {
		const CDoubleMatrix legacy_m = data.m_vara.MarCnData[idcnMcp][c];
		const CDoubleVector legacy_b = data.m_vara.corner_vector(idcnRHS, c);
		if (std::fabs(data.nodal.master.M[c][0] - legacy_m.xx) > tol ||
			std::fabs(data.nodal.master.M[c][1] - legacy_m.xy) > tol ||
			std::fabs(data.nodal.master.M[c][2] - legacy_m.yx) > tol ||
			std::fabs(data.nodal.master.M[c][3] - legacy_m.yy) > tol ||
			std::fabs(data.nodal.master.b[2 * c] - legacy_b.x) > tol ||
			std::fabs(data.nodal.master.b[2 * c + 1] - legacy_b.y) > tol) {
			return false;
		}
	}
	return true;
