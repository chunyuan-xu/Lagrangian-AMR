...
namespace HydroCallbacks {
	...
	void quadrant_compute_corner_force_callback(p4est_iter_volume_info_t* info, void* user_data)
	{
		...
		for (int k = 0; k < CNDIM; k++)
		{
			...
			m_plus->pi = m_vara->DouCnData[idReconstructPressure][k] - m_plus->Zcp * (DeltaU[k] ^ m_plus->Ncp);
			m_minus->pi = m_vara->DouCnData[idReconstructPressure][k] - m_minus->Zcp * (DeltaU[k] ^ m_minus->Ncp);
			...
			if (data->points[k].IsHanging == true)
			{
				CDoubleVector DeltaUa = data->points[k].master_velocity_lag[0] - m_vara->VecCnData[idReconstructVelocity][k];
				CDoubleVector DeltaUb = data->points[k].master_velocity_lag[1] - m_vara->VecCnData[idReconstructVelocity][k];
				m_plus->pi_master[0] = 0.5 * (m_vara->DouCnData[idReconstructPressure][k] - m_plus->Zcp * (DeltaUa ^ m_plus->Ncp));
				m_plus->pi_master[1] = 0.5 * (m_vara->DouCnData[idReconstructPressure][k] - m_plus->Zcp * (DeltaUb ^ m_plus->Ncp));
				m_minus->pi_master[0] = 0.5 * (m_vara->DouCnData[idReconstructPressure][k] - m_minus->Zcp * (DeltaUa ^ m_minus->Ncp));
				m_minus->pi_master[1] = 0.5 * (m_vara->DouCnData[idReconstructPressure][k] - m_minus->Zcp * (DeltaUb ^ m_minus->Ncp));

				m_vara->VecCnData[idcnFcp][k] = CDoubleVector(0., 0.);
				m_vara->DouCnData[idcnWork][k] = 0.;
			}
			if (data->points[k].IsHanging == false)
			{
				...
				m_vara->DouCnData[idcnWork][k] = m_vara->VecCnData[idcnVelocity_lag][k] ^ m_vara->VecCnData[idcnFcp][k];
			}
	...
	void quadrant_hanging_solver_callback(p4est_iter_face_info_t* info, void* user_data)
	{
		GhostCallbackContext* context =
			static_cast<GhostCallbackContext*>(user_data);
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (side[i]->is_hanging == Hanging)
			{
				...
				m_child1_data->points[m_which_corner[0]].master_velocity_lag[0] = master_velocity[0];
				m_child1_data->points[m_which_corner[0]].master_velocity_lag[1] = master_velocity[1];
				m_child2_data->points[m_which_corner[1]].master_velocity_lag[0] = master_velocity[1];
				m_child2_data->points[m_which_corner[1]].master_velocity_lag[1] = master_velocity[0];
	...
	void
		quadrant_matrix_from_nonconforming_interface_callback(p4est_iter_face_info_t* info, void* user_data)
	{
		p4est_t* p4est = info->p4est;
		p4est_data_t* p4est_data = &((P4estBridge*)info->p4est->user_pointer)->data;
		GhostCallbackContext* context =
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (side[i]->is_hanging == Hanging)
			{
				...
				CDoubleMatrix M1, M2, M3, M4;
				M1 = 0.5 * m_quad_data->m_vara.MarCnData[idcnMcp][m_which_corner[0]];
				M2 = 0.5 * m_quad_data_aside->m_vara.MarCnData[idcnMcp][m_which_corner[1]];
				M3 = 0.5 * m_quad_data->m_vara.MarCnData[idcnMcp][m_which_corner[0]];
				M4 = 0.5 * m_quad_data_aside->m_vara.MarCnData[idcnMcp][m_which_corner[1]];

				CDoubleMatrix sum12 = M1 + M2;
				m_quad_data->m_vara.MarCnData[idcnMch][m_master_corner[0]] += sum12;
				m_quad_data->m_vara.VecCnData[idcnRHS_h][m_master_corner[0]] +=
					0.5 * m_quad_data->m_vara.VecCnData[idcnRHS][m_which_corner[0]] +
					0.5 * m_quad_data_aside->m_vara.VecCnData[idcnRHS][m_which_corner[1]];

				CDoubleMatrix sum34 = M3 + M4;
				m_quad_data_aside->m_vara.MarCnData[idcnMch][m_master_corner[1]] += sum34;
				m_quad_data_aside->m_vara.VecCnData[idcnRHS_h][m_master_corner[1]] +=
					0.5 * m_quad_data->m_vara.VecCnData[idcnRHS][m_which_corner[0]] +
					0.5 * m_quad_data_aside->m_vara.VecCnData[idcnRHS][m_which_corner[1]];
