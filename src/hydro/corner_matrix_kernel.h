...
namespace HydroCallbacks {
	...
	{
		double Tc = 0.;
		double DeltaUDotNcpPlus, DeltaUDotNcpMinus;
		DeltaUDotNcpPlus = 0.0;
		DeltaUDotNcpMinus = 0.0;
		...
		double compression = DeltaU ^ LcpNcp;
		if (compression < -m_eps)
		{
			Tc = 1.44;
			DeltaUDotNcpPlus = fabs(DeltaU ^ m_plus.Ncp);
			DeltaUDotNcpMinus = fabs(DeltaU ^ m_minus.Ncp);
		}
		...
		m_plus.Zcp = vara.corner(idReconstructDensity, k) * vara.cell(idSoundSpeed) + Tc * vara.corner(idReconstructDensity, k) * DeltaUDotNcpPlus;
		m_minus.Zcp = vara.corner(idReconstructDensity, k) * vara.cell(idSoundSpeed) + Tc * vara.corner(idReconstructDensity, k) * DeltaUDotNcpMinus;
