...
namespace AMRCallbacks {
	...
	void quadrant_edge_minmod_estimate_callback(p4est_iter_face_info_t* info, void* user_data)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (side[i]->is_hanging == Hanging)
			{
				...
				child1_gradient = hanging_gradient(
					parent_para, child1_para, parent_center, child1_center);
				child2_gradient = hanging_gradient(
					parent_para, child2_para, parent_center, child2_center);
	...
	void quadrant_set_init_children_callback(p4est_iter_face_info_t* info, void* user_data)
	{
		GhostCallbackContext* context =
			static_cast<GhostCallbackContext*>(user_data);
		p4est_t* p4est = info->p4est;
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (side[i]->is_hanging == Hanging)
			{
				...
				CHalf_edge_data* m_child1_master1, * m_child1_master2, * m_child2_master1, * m_child2_master2;
				m_child1_master1 = (CHalf_edge_data*)&m_child1_cndata[m_master_corner[0]].hdata[1 - m_which_side[0]];
				m_child1_master2 = (CHalf_edge_data*)&m_child1_cndata[m_unconstrained_master_corner[0]].hdata[m_which_side[0]];
				m_child2_master1 = (CHalf_edge_data*)&m_child2_cndata[m_master_corner[1]].hdata[1 - m_which_side[1]];
				m_child2_master2 = (CHalf_edge_data*)&m_child2_cndata[m_unconstrained_master_corner[1]].hdata[m_which_side[1]];
				m_child1_master1->Lcp = m_child1_master1->Lcp * 2.;
				m_child1_master2->Lcp = m_child1_master2->Lcp * 2.;
				m_child2_master1->Lcp = m_child2_master1->Lcp * 2.;
				m_child2_master2->Lcp = m_child2_master2->Lcp * 2.;
			}
		}
	}
	...
	void Get_AMR_BDY_info(p4est_t* p4est, GhostSession& session)
	{
		...
		p4est_iterate(p4est,
			...
			&callback_context,
			NULL,
			AMRCallbacks::quadrant_set_init_children_callback,
#ifdef P4_TO_P8
			NULL,
			...
			p4est_quadrant_t* incoming[])
	{
		...
		else
		{
			...
			for (int i = 0; i < P4EST_CHILDREN; i++)
			{
				...
				reset_refine_child_parent_edge_scratch(*child_data);
