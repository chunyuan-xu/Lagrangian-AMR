...
| M9.4.1 | main.cpp 终极编排瘦身（`advance_time_step`→`Simulation`，压至数百行） | `Simulation` | ✅ `待提交` |
...
- `golden-gates-m9-4-1-2026-08-13.md`：M9.4.1

## 3.1 M9 收口状态

**M9 全部子里程碑已收口**（M9.1.3/1.4/2.3/2.4/3.2/3.3/4.1），main.cpp 从 M8 的 2455 行减至 **346 行**（启动骨架 + `Simulation::run` 编排），达成"压至数百行"目标。G2 全程 retired。M9 后待处理：`quadtree_static`（185 行）归属识别（未纳入 M9 范围）。
...
## 5. 剩余壳函数清单（main.cpp 当前 346 行）

| 函数 | 行数 | 归属 | 计划项 |
|---|---|---|---|
| `quadtree_static` | 185 | 待识别，勿预设 | 未纳入 M9 |

> **M9 已全部收口**：所有包装壳函数已迁至 `AMRCallbacks`/`HydroCallbacks`/`HydroController`/`IOCallbacks`/`Simulation`/`Initializer`/`core/trace.h`。main.cpp 仅留启动骨架。

## 6. 下一步建议

1. 识别 `quadtree_static`（185 行）用途，决定归属或清理（M9 范围外）。
2. 若继续推进整体重构，下一步为 M10 或回归审计；更新本文件后推进。
