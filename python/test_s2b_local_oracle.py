"""S2b micro-gate: local master oracle/identity comparison."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-s2b-local-oracle.json"
...
int main()
{
	...
	for (int c = 0; c < 4; ++c) {
		cell.m_vara.MarCnData[idcnMcp][c] = CDoubleMatrix(0.5, 0.0, 0.0, 0.5);
	}
	cell.m_vara.corner_vector(idcnRHS, 0) = CDoubleVector(0.5, -0.5);
	cell.m_vara.corner_vector(idcnRHS, 1) = CDoubleVector(0.5, 0.5);
	cell.m_vara.corner_vector(idcnRHS, 2) = CDoubleVector(-0.5, 0.5);
	cell.m_vara.corner_vector(idcnRHS, 3) = CDoubleVector(-0.5, -0.5);

	Nodal::write_cell_local_master(cell);
	assert(Nodal::verify_local_master_against_legacy(cell));

	cell.nodal.master.b[0] += 1.0;
	assert(!Nodal::verify_local_master_against_legacy(cell));
	return 0;
}
"""
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.s2b-local-oracle.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "regular_matches_legacy": True,
    ...
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="s2b-oracle-", dir=ROOT / ".tmp") as directory:
