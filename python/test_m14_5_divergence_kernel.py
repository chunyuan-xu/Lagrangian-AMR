"""M14.5 micro-gate: divergence kernel."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-m14-5-divergence-kernel.json"
...
int main()
{
    ...
    CDoubleVector vel[CNDIM] = {
        CDoubleVector(0.1, 0.1),
        CDoubleVector(0.1, 0.1),
        CDoubleVector(0.1, 0.1),
        CDoubleVector(0.1, 0.1)
    };
    double d = HydroCallbacks::compute_divergence(
        p4est_data_t::MyCoordType::plane, coord, vel);
    assert(std::fabs(d) < 1e-12);
    return 0;
}
"""
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.m14-5-divergence-kernel.v1",
    ...
    with tempfile.TemporaryDirectory(prefix="m14-5-", dir=ROOT / ".tmp") as directory:
    ...
    args.summary.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(f"MG-M14-5 {summary['status']}: {args.summary}")
