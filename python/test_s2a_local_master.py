"""S2a micro-gate: cell-local DBGF master shadow writer."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-s2a-local-master.json"
...
int main()
{
	...
	// Corner LB block: M=diag(0.5,0.5), b=(0.5,-0.5).
	assert(std::fabs(cell.nodal.master.M[0][0] - 0.5) < 1e-12);
	assert(std::fabs(cell.nodal.master.M[1][1] - 0.5) < 1e-12);
	assert(std::fabs(cell.nodal.master.M[0][1]) < 1e-12);
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.s2a-local-master.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "regular_fixture": True,
        "block_diagonal_written": True,
        "status": "ERROR",
    }
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="s2a-master-", dir=ROOT / ".tmp") as directory:
