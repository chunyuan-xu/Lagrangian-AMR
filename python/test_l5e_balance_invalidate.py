"""L5e micro-gate: MG-AMR balance stamp invalidation."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-l5e-balance-invalidate.json"
...
int main()
{
	p4est_data_t data = {};
	data.current_step = 7;
	EpochContext ctx = Nodal::make_stage_context(data, 9, 4, 0, StagePhase::Assemble);
	quad_data_t cell = {};
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.l5e-balance-invalidate.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "balance_invalidates": True,
        "stage_restores": True,
        "status": "ERROR",
    }
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="l5e-balance-", dir=ROOT / ".tmp") as directory:
