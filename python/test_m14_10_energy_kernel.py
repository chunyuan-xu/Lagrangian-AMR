"""M14.10 micro-gate: total/internal energy update kernel."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-m14-10-energy-kernel.json"
...
{
    CVariable vara = {};
    vara.cell(idTotalEnergy_half) = 10.0;
    vara.cell(idInternalEnergy_half) = 5.0;
    vara.cell(idTotalWork) = 2.0;
    vara.cell(idKineticVariation) = 1.0;
    vara.cell(idMass) = 2.0;
    HydroCallbacks::update_energy(vara, 0.5, 0, 7);
    assert(std::fabs(vara.cell(idTotalEnergy_lag) - 9.5) < 1e-12);
    assert(std::fabs(vara.cell(idInternalEnergy_lag) - 4.75) < 1e-12);
    return 0;
}
"""
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.m14-10-energy-kernel.v1",
    ...
    with tempfile.TemporaryDirectory(prefix="m14-10-", dir=ROOT / ".tmp") as directory:
    ...
    args.summary.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(f"MG-M14-10 {summary['status']}: {args.summary}")
