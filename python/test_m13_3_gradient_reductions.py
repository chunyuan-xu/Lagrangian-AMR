"""M13.3 micro-gate: max gradient reductions."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-m13-3-gradient-reductions.json"
...
int main()
{
    assert(AMRCallbacks::reduce_max_edge_to_cell(1.0, 3.0) == 3.0);
    assert(AMRCallbacks::reduce_max_edge_to_cell(4.0, 2.0) == 4.0);
    assert(AMRCallbacks::reduce_max_corner_neighbor(0.5, 0.25) == 0.5);
    assert(AMRCallbacks::reduce_max_corner_neighbor(0.5, 1.0) == 1.0);
    return 0;
}
"""
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.m13-3-gradient-reductions.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "reductions_extracted": True,
        "status": "ERROR",
    }
    if not CXX.exists():
    ...
    with tempfile.TemporaryDirectory(prefix="m13-3-", dir=ROOT / ".tmp") as directory:
    ...
    args.summary.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(f"MG-M13-3 {summary['status']}: {args.summary}")
