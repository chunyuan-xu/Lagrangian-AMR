"""S2c micro-gate: publication boundary stamp validation."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-s2c-publication-validation.json"
...
int main()
{
	...
	EpochContext ctx = Nodal::make_stage_context(data, 13, 6, 0, StagePhase::Assemble);
	quad_data_t cell = {};
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.s2c-publication-validation.v1",
    ...
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="s2c-pub-", dir=ROOT / ".tmp") as directory:
