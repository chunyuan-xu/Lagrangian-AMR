"""S5d micro-gate: MG-SOLVE master velocity publication validation."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-s5d-master-exchange.json"
...
int main()
{
	...
	EpochContext ctx = Nodal::make_stage_context(data, 15, 8, 0, StagePhase::Solve);
	quad_data_t cell = {};
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.s5d-master-exchange.v1",
    ...
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="s5d-master-", dir=ROOT / ".tmp") as directory:
