"""S6b micro-gate: hanging branch pressure/force recovery."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-s6b-branch-force.json"
...
int main()
{
	...
	in.e[0] = HalfEdgeInput{0.4, {1.0, 0.0}};
	in.e[1] = HalfEdgeInput{0.6, {1.0, 0.0}};

	LocalCornerOutput out = Nodal::build_local_corner(in);
	assert(std::fabs(out.branch_pressure[0] - 0.0) < 1e-12);
	assert(std::fabs(out.branch_pressure[1] - 0.0) < 1e-12);
	assert(std::fabs(out.branch_force[0][0]) < 1e-12);
	assert(std::fabs(out.branch_force[1][0]) < 1e-12);
	return 0;
}
"""
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.s6b-branch-force.v1",
    ...
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="s6b-branch-", dir=ROOT / ".tmp") as directory:
