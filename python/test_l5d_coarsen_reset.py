"""L5d micro-gate: MG-AMR coarsen parent stamp invalidation."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-l5d-coarsen-reset.json"
...
int main()
{
	p4est_data_t data = {};
	data.current_step = 6;
	EpochContext ctx = Nodal::make_stage_context(data, 8, 3, 0, StagePhase::Assemble);
	quad_data_t cell = {};
	Nodal::stamp_stage_reset(cell, ctx);
	assert(!Nodal::validate_stage_reset(cell, ctx).failed);

	// Coarsen-created parent is reset and explicitly invalidated.
	Nodal::reset_storage(cell.nodal);
	Nodal::invalidate_stage_reset(cell);
	assert(Nodal::validate_stage_reset(cell, ctx).failed);

	// A later stage reset restores validity.
	Nodal::stamp_stage_reset(cell, ctx);
	assert(!Nodal::validate_stage_reset(cell, ctx).failed);
	return 0;
}
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.l5d-coarsen-reset.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "coarsen_reset_invalidates": True,
        "stage_restores": True,
        "status": "ERROR",
    }
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="l5d-coarsen-", dir=ROOT / ".tmp") as directory:
