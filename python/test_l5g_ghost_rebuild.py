"""L5g micro-gate: MG-AMR ghost rebuild stamp invalidation."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-l5g-ghost-rebuild.json"
...
int main()
{
	p4est_data_t data = {};
	data.current_step = 9;
	EpochContext ctx = Nodal::make_stage_context(data, 11, 5, 0, StagePhase::Assemble);
	quad_data_t cell = {};
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.l5g-ghost-rebuild.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "ghost_rebuild_invalidates": True,
        "stage_restores": True,
        "status": "ERROR",
    }
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="l5g-ghost-", dir=ROOT / ".tmp") as directory:
