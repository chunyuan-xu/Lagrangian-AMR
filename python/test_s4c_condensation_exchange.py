"""S4c micro-gate: MG-CONDENSE publication exchange stamp validation."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-s4c-condensation-exchange.json"
...
int main()
{
	...
	EpochContext ctx = Nodal::make_stage_context(data, 14, 7, 0, StagePhase::Assemble);
	quad_data_t cell = {};
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.s4c-condensation-exchange.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "valid_after_publication": True,
        "stale_rejected": True,
        "status": "ERROR",
    }
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="s4c-cond-", dir=ROOT / ".tmp") as directory:
