"""L5b micro-gate: MG-EPOCH geometry epoch invalidation."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-l5b-geometry-epoch.json"
...
int main()
{
	p4est_data_t data = {};
	data.current_step = 4;
	EpochContext ctx = Nodal::make_stage_context(data, 6, 2, 1, StagePhase::Assemble);
	quad_data_t cell = {};
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.l5b-geometry-epoch.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "invalidate_before_refresh": True,
        "stamp_after_refresh": True,
        "status": "ERROR",
    }
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="l5b-geom-", dir=ROOT / ".tmp") as directory:
