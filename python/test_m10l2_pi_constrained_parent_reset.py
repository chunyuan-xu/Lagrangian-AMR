"""M10L.2 micro-gate: pi_constrained_parent read-before-write repair."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-m10l2-pi-constrained-parent-reset.json"
...
int main()
{
    ...
    for (int cnid = 0; cnid < CNDIM; ++cnid) {
        cell.points[cnid].pi_constrained_parent = poison;
    }

    AMRCallbacks::reset_point_pi_constrained_parent(cell);

    for (int cnid = 0; cnid < CNDIM; ++cnid) {
        const double value = cell.points[cnid].pi_constrained_parent;
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.m10l2-pi-constrained-parent.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "field_domain": "CPoint_data_t::pi_constrained_parent",
        "required_writer": "AMRCallbacks::reset_point_pi_constrained_parent",
        "status": "ERROR",
    }
    if not CXX.exists():
    ...
    with tempfile.TemporaryDirectory(prefix="m10l2-pi-", dir=ROOT / ".tmp") as directory:
    ...
    args.summary.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(f"MG-M10L2 {summary['status']}: {args.summary}")
