"""M13.2 micro-gate: hanging coarse/fine gradient kernel."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-m13-2-hanging-gradient.json"
...
int main()
{
    CDoubleVector parent_center(0.0, 0.0);
    CDoubleVector child_center(2.0, 0.0);
    double g = AMRCallbacks::hanging_gradient(2.0, 4.0, parent_center, child_center);
    assert(std::fabs(g - 1.0) < 1e-12);
    return 0;
}
"""
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.m13-2-hanging-gradient.v1",
    ...
    with tempfile.TemporaryDirectory(prefix="m13-2-", dir=ROOT / ".tmp") as directory:
    ...
    args.summary.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(f"MG-M13-2 {summary['status']}: {args.summary}")
