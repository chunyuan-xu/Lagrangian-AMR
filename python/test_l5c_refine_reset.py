"""L5c micro-gate: MG-AMR refine child stamp invalidation."""
...
DEFAULT_SUMMARY = ROOT / ".tmp" / "mg-l5c-refine-reset.json"
...
int main()
{
	p4est_data_t data = {};
	data.current_step = 5;
	EpochContext ctx = Nodal::make_stage_context(data, 7, 3, 0, StagePhase::Assemble);
	quad_data_t cell = {};
	Nodal::stamp_stage_reset(cell, ctx);
	assert(!Nodal::validate_stage_reset(cell, ctx).failed);

	// Refine-created children are reset and explicitly invalidated.
...
def main():
    ...
    summary = {
        "schema": "lagrangian-amr.micro-gate.l5c-refine-reset.v1",
        "started_at": datetime.now().astimezone().isoformat(),
        "refine_reset_invalidates": True,
        "stage_restores": True,
        "status": "ERROR",
    }
    if not CXX.exists():
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="l5c-refine-", dir=ROOT / ".tmp") as directory:
