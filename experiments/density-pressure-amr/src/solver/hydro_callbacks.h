...
void quadrant_update_energy_callback(p4est_iter_volume_info_t *info, void *user_data)
{
    ...
    if(std::getenv("AMR_STARTUP_ENERGY_AUDIT") && p4est_data->current_step<=20 && info->quad->level>=6) {
        const auto &v=*m_vara;const auto uc=.5*(v.cell_vector(idCentroidVelo_half)+v.cell_vector(idCentroidVelo_lag));double heat[4]={};
        for(int c=0;c<4;++c){const auto du=v.corner_vector(idcnVelocity_lag,c)-uc;heat[0]+=du^v.corner_vector(idcnFcp,c);heat[1]+=du^v.corner_vector(idcnFluxRelaxed,c);const auto&pc=data->m_pc_edge_data[c];if(pc.IsParentChildBoun){const auto dp=pc.Hanging_velocity-uc;heat[2]+=dp^v.corner_vector(ideFcp,c);heat[3]+=dp^pc.FluxRelaxed;}}
        const double scale=p4est_data->dt_iter/v.cell(idMass);
        std::printf("START_ENERGY step=%d t=%.17g tree=%d level=%d x=%d y=%d dt=%.17g mass=%.17g ecur=%.17g ehalf=%.17g ecandidate=%.17g Ehalf=%.17g Enew=%.17g work=%.17g kinetic=%.17g de_corner=%.17g de_relax=%.17g de_parent=%.17g de_parent_relax=%.17g\n",p4est_data->current_step,p4est_data->current_time,(int)info->treeid,(int)info->quad->level,info->quad->x,info->quad->y,p4est_data->dt_iter,v.cell(idMass),v.cell(idInternalEnergy_cur),v.cell(idInternalEnergy_half),v.cell(idInternalEnergy_half)-scale*(v.cell(idTotalWork)-v.cell(idKineticVariation)),v.cell(idTotalEnergy_half),v.cell(idTotalEnergy_lag),v.cell(idTotalWork),v.cell(idKineticVariation),-scale*heat[0],-scale*heat[1],-scale*heat[2],-scale*heat[3]);
    }
