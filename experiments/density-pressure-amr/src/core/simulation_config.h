...
inline bool valid(const MeshConfig &config)
{
	return config.refine_criterion >= 0 &&
		config.refine_criterion <= 6 &&
		(config.density_gradient_method == 0 || config.density_gradient_method == 1) &&
		config.global_nx > 0 &&
		config.global_ny > 0 &&
		...
		(config.refine_criterion != 6 ||
			(config.refine_error > 0.0 &&
			 config.coarsen_error >= 0.0 &&
			 config.coarsen_error < config.refine_error)) &&
		...
		(!config.initial_annulus_mesh ||
			(std::isfinite(config.initial_annulus_center_x) &&
			 std::isfinite(config.initial_annulus_center_y) &&
			 std::isfinite(config.initial_annulus_inner_radius) &&
			 std::isfinite(config.initial_annulus_outer_radius) &&
			 config.initial_annulus_inner_radius >= 0.0 &&
			 config.initial_annulus_outer_radius >
				 config.initial_annulus_inner_radius &&
			 config.initial_annulus_level >= config.minimum_level &&
			 config.initial_annulus_level <= config.maximum_level)) &&
