...
struct p4est_data_t {
	...
	int density_gradient_method; // 0: legacy max-edge; 1: weighted least squares
	int minus_level;   
	int max_level;     
	bool initial_annulus_mesh;
	double initial_annulus_center_x;
	double initial_annulus_center_y;
	double initial_annulus_inner_radius;
	double initial_annulus_outer_radius;
	int initial_annulus_level;
	int  refine_period;  
	...
	SimulationModel::SimulationConfig simulation_config() const {
		return SimulationModel::SimulationConfig{
			...
			SimulationModel::MeshConfig{
				...
				minus_level,
				max_level,
				initial_annulus_mesh,
				initial_annulus_center_x,
				initial_annulus_center_y,
				initial_annulus_inner_radius,
				initial_annulus_outer_radius,
				initial_annulus_level,
				refine_coarsen_enum,
				density_gradient_method,
				refine_err,
				coarsen_error,
				refine_period,
				repartition_period,
	...
	void load_from_config(const IOAlgorithm::ConfigParser& cfg) {
		...
		if (cfg.HasKey("density_gradient_method")) density_gradient_method = cfg.GetInt("density_gradient_method", density_gradient_method);
		...
		if (cfg.HasKey("initial_annulus_mesh")) initial_annulus_mesh = cfg.GetInt("initial_annulus_mesh", initial_annulus_mesh ? 1 : 0) != 0;
		if (cfg.HasKey("initial_annulus_center_x")) initial_annulus_center_x = cfg.GetDouble("initial_annulus_center_x", initial_annulus_center_x);
		if (cfg.HasKey("initial_annulus_center_y")) initial_annulus_center_y = cfg.GetDouble("initial_annulus_center_y", initial_annulus_center_y);
		if (cfg.HasKey("initial_annulus_inner_radius")) initial_annulus_inner_radius = cfg.GetDouble("initial_annulus_inner_radius", initial_annulus_inner_radius);
		if (cfg.HasKey("initial_annulus_outer_radius")) initial_annulus_outer_radius = cfg.GetDouble("initial_annulus_outer_radius", initial_annulus_outer_radius);
		if (cfg.HasKey("initial_annulus_level")) initial_annulus_level = cfg.GetInt("initial_annulus_level", initial_annulus_level);
