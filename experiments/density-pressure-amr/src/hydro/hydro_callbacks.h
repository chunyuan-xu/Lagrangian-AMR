...
void quadrant_relaxed_hanging_solver_callback(p4est_iter_face_info_t *info, void *user_data)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (side[i]->is_hanging == Hanging)
		{
			...
			// Isolated experiment: redistribute the SAME constrained-node residual
			// according to thermal capacity. This is not a positivity proof.
			static const bool thermal_relax_weights = []() {
				const char *value = std::getenv("AMR_THERMAL_RELAX_WEIGHTS");
				return value && std::atoi(value) == 1;
			}();
			if (thermal_relax_weights) {
				child1_total_energy = m_child1_read_vara->cell(idMass) * m_child1_read_vara->cell(idInternalEnergy_cur);
				child2_total_energy = m_child2_read_vara->cell(idMass) * m_child2_read_vara->cell(idInternalEnergy_cur);
				parent_total_energy = m_parent_read_vara->cell(idMass) * m_parent_read_vara->cell(idInternalEnergy_cur);
				SC_CHECK_ABORT(std::isfinite(child1_total_energy) && child1_total_energy > 0. &&
					std::isfinite(child2_total_energy) && child2_total_energy > 0. &&
					std::isfinite(parent_total_energy) && parent_total_energy > 0. &&
					std::isfinite(child1_total_energy + child2_total_energy + parent_total_energy),
					"Thermal relaxation weights require positive finite thermal energies");
			}


            double relax_denominator=child1_total_energy+child2_total_energy+parent_total_energy;
            auto relax_weights=DissipativeWeightMath::A{{child1_total_energy/relax_denominator,child2_total_energy/relax_denominator,parent_total_energy/relax_denominator}};
            std::array<CDoubleVector,3> relax_forces={{relax_weights[0]*Flux_relaxed,relax_weights[1]*Flux_relaxed,relax_weights[2]*Flux_relaxed}};
            if(DissipativeWeightsExperiment::mode()){
                const CVariable* reads[3]={m_child1_read_vara,m_child2_read_vara,m_parent_read_vara};
                const CDoubleMatrix matrices[3]={m_child1_read_vara->MarCnData[idcnMcp][m_which_corner[0]],m_child2_read_vara->MarCnData[idcnMcp][m_which_corner[1]],m_parent_read_vara->MarCnData[ideMcp][parent_face_index]};
                relax_forces=DissipativeWeightsExperiment::forces(p4est,reads,matrices,hanging_velo,Flux_relaxed,relax_weights,g_trace_riemann_iter);
            }
			if (!side[i]->is.hanging.is_ghost[0]) {
				m_child1_vara->corner_vector(idcnFluxRelaxed, m_which_corner[0]) = relax_forces[0];
			}
			if (!side[i]->is.hanging.is_ghost[1]) {
				m_child2_vara->corner_vector(idcnFluxRelaxed, m_which_corner[1]) = relax_forces[1];
			}
			if (!side[full_index]->is.full.is_ghost) {
				...
				PCInfo[parent_face_index].FluxRelaxed = relax_forces[2];
			}
		}
	}
}
