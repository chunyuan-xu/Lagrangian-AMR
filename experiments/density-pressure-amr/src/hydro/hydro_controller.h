...
#include "diagnostics/derived_state_experiment.h"
#include "diagnostics/hydro_budget_audit.h"
#include "diagnostics/riemann_iteration_experiment.h"
...
void RiemannSolver(p4est_t * p4est, GhostSession &session)
{
    ...
    DissipativeWeightsExperiment::begin(p4est);
    DerivedStateExperiment::record(p4est,"before_riemann_matrix");
    ...
    RiemannIterationExperiment::probe_or_apply(p4est,session,[&](){
        RiemannPhases::run_iteration(p4est,session,MatrixAssemble,ComputeCornerNodeVelocity,
            ComputeHangingNodeVelocityUsingConstrainedConditionByMasterNodes);
    });
	ComputeCornerAndEdgeForce(p4est);
    DissipativeWeightsExperiment::finish(p4est);
}
...
{
	p4est_data_t *p4est_data = &((P4estBridge *)p4est->user_pointer)->data;
	if (p4est_data->density_gradient_method == 1 &&
		(p4est_data->refine_coarsen_enum == RefineCriteria::DensityGradient ||
		 p4est_data->refine_coarsen_enum == RefineCriteria::DimensionlessDensityGradient)) {
		const size_t invalid = DensityGradientEstimator::estimate(p4est, session);
		static bool warned_invalid = false;
		if (invalid && !warned_invalid) {
			P4EST_LERRORF("[density-gradient] %llu local invalid least-squares fits; "
				"using infinite indicators (warning once per rank).\n",
