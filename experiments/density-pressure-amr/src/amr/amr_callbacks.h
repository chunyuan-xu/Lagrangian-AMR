#pragma once
#include "diagnostics/transfer_energy_audit.h"
#include <algorithm>
#include <cstdlib>
#include <p4est.h>
#include "defines.h"
...
void quadrant_edge_minmod_estimate_callback(p4est_iter_face_info_t *info, void *user_data)
{
	...
	case RefineCriteria::DimensionlessDensityGradient:
	...
	p4est_quadrant_t *incoming[])
{
    ...
    if(std::getenv("AMR_ORIGIN_AUDIT")) {
        SC_CHECK_ABORT(p4est->mpisize==1,"Origin log requires a serial experiment");
        for(int k=0;k<num_incoming;++k) if(incoming[k]->level>=6)
            std::printf("ORIGIN stage=%s action=%s step=%d t=%.17g tree=%d level=%d x=%lld y=%lld\n",PressureExperiment::stage(),num_outgoing>1?"merge":"split",p4est_data->current_step,p4est_data->current_time,(int)which_tree,(int)incoming[k]->level,(long long)incoming[k]->x,(long long)incoming[k]->y);
    }

    if(std::getenv("AMR_FRONT_DEPENDENCY_AUDIT") && num_outgoing==1 && outgoing[0]->level==6) {
        const auto&q=*outgoing[0];const auto&v=static_cast<const quad_data_t*>(q.p.user_data)->m_vara;
        // Pre-transfer parent data, before inherited child values exist.
        const double eta=std::sqrt(v.cell(idVolume))*std::abs(v.cell(idCDensityGradient))/std::max(std::abs(v.cell(idDensity_cur)),static_cast<double>(m_eps));
        std::printf("DEPENDENCY_SPLIT stage=%s step=%d t=%.17g tree=%d level=%d x=%lld y=%lld rho=%.17g p=%.17g eta=%.17g jp=%.17g compression=%.17g\n",
            PressureExperiment::stage(),p4est_data->current_step,p4est_data->current_time,(int)which_tree,(int)q.level,(long long)q.x,(long long)q.y,v.cell(idDensity_cur),v.cell(idPressure_cur),eta,v.cell(idAMRPressureJump),CompressionExperiment::indicator(v));
    }

	if (num_outgoing > 1)
	{
        if(PressureExperiment::trace() && incoming[0]->level==5 && incoming[0]->x==637534208 && incoming[0]->y==603979776) std::printf("PTRACE MERGE step=%d t=%.17g\n",p4est_data->current_step,p4est_data->current_time);
        ...
        TransferEnergyAudit::merge(p4est,which_tree,incoming[0],outgoing,PressureExperiment::stage());
        if(std::getenv("AMR_STARTUP_ENERGY_AUDIT") && p4est_data->current_step<=20) {
            const auto&v=parent_data->m_vara;double child_mass=0.,child_ie=0.,child_ke=0.;
            for(int c=0;c<4;++c){const auto&w=static_cast<quad_data_t*>(outgoing[c]->p.user_data)->m_vara;const auto u=w.cell_vector(idCentroidVelo_cur);child_mass+=w.cell(idMass);child_ie+=w.cell(idMass)*w.cell(idInternalEnergy_cur);child_ke+=w.cell(idMass)*.5*(u.x*u.x+u.y*u.y);}
            const auto u=v.cell_vector(idCentroidVelo_cur);std::printf("START_MERGE step=%d t=%.17g stage=%s tree=%d level=%d x=%d y=%d child_e=%.17g parent_e=%.17g child_ke=%.17g parent_ke=%.17g p=%.17g\n",p4est_data->current_step,p4est_data->current_time,PressureExperiment::stage(),(int)which_tree,(int)incoming[0]->level,incoming[0]->x,incoming[0]->y,child_ie/child_mass,v.cell(idInternalEnergy_cur),child_ke/child_mass,.5*(u.x*u.x+u.y*u.y),v.cell(idPressure_cur));
        }
        if(PressureExperiment::target_family(incoming[0]->level,incoming[0]->x,incoming[0]->y)) {
            double mass=0.,ie=0.,ke=0.,total=0.,volume=0.,pmin=1e300,pmax=0.;
            for(int i=0;i<4;++i) {
                const auto &v=static_cast<quad_data_t *>(outgoing[i]->p.user_data)->m_vara;
                const auto &u=v.cell_vector(idCentroidVelo_cur);
                const double m=v.cell(idMass);
                mass+=m;ie+=m*v.cell(idInternalEnergy_cur);ke+=.5*m*(u.x*u.x+u.y*u.y);
                total+=m*v.cell(idTotalEnergy_cur);volume+=v.cell(idVolume);
                pmin=std::min(pmin,v.cell(idPressure_cur));pmax=std::max(pmax,v.cell(idPressure_cur));
            }
            const auto &v=parent_data->m_vara;const auto &u=v.cell_vector(idCentroidVelo_cur);
            std::printf("MERGE_ENERGY step=%d t=%.17g x=%lld y=%lld child_ie=%.17g parent_ie=%.17g kinetic_loss=%.17g state_residual=%.17g total_residual=%.17g volume_ratio=%.17g child_pmin=%.17g child_pmax=%.17g parent_p=%.17g\n",p4est_data->current_step,p4est_data->current_time,(long long)incoming[0]->x,(long long)incoming[0]->y,ie/mass,v.cell(idInternalEnergy_cur),ke/mass-.5*(u.x*u.x+u.y*u.y),(total-ie-ke)/mass,v.cell(idMass)*v.cell(idTotalEnergy_cur)-total,v.cell(idVolume)/volume,pmin,pmax,v.cell(idPressure_cur));
        }
	}
	else
	{
        ...
        if(PressureExperiment::trace() && outgoing[0]->level==5 && outgoing[0]->x==637534208 && outgoing[0]->y==603979776) std::printf("PTRACE SPLIT phase=%s step=%d t=%.17g rho=%.17g pressure=%.17g eta=%.17g jp=%.17g\n",PressureExperiment::stage(),p4est_data->current_step,p4est_data->current_time,parent_data->m_vara.cell(idDensity_cur),parent_data->m_vara.cell(idPressure_cur),AMRAgorithm::DimensionlessDensityGradientIndicator(parent_data->m_vara),parent_data->m_vara.cell(idAMRPressureJump));
        if(PressureExperiment::target_family(outgoing[0]->level,outgoing[0]->x,outgoing[0]->y)) std::printf("FAMILY_SPLIT phase=%s step=%d t=%.17g x=%lld y=%lld eta=%.17g jp=%.17g rho=%.17g p=%.17g\n",PressureExperiment::stage(),p4est_data->current_step,p4est_data->current_time,(long long)outgoing[0]->x,(long long)outgoing[0]->y,AMRAgorithm::DimensionlessDensityGradientIndicator(parent_data->m_vara),parent_data->m_vara.cell(idAMRPressureJump),parent_data->m_vara.cell(idDensity_cur),parent_data->m_vara.cell(idPressure_cur));
